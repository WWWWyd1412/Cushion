...
class AcmdFprStepByStep(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("ACMD-FPR 逻辑验证工具 - 自适应时频版")
    ...
    def setup_ui(self):
        ...
        self.btn_3 = QPushButton("步骤 3: ACMD 自适应能量流剥离 (无需寻优 K)")
        self.btn_4 = QPushButton("步骤 4: 重构呼吸波形 (单分量极大值 vs 多成分 SNR)")
    ...
    def run_step_3(self):
        ...
        energies = [np.sqrt(np.mean(comp ** 2)) for comp in self.components_cache]

        ax_energy = self.figure.add_subplot(111)
        ax_energy.bar(k_range, energies, color='#2c3e50', alpha=0.8, width=0.5)
        ax_energy.set_title("ACMD 各级自适应啁啾模态有效能量分布 (降序排序)")
        ...
        for idx, eng in enumerate(energies):
            self.log(f"Mode {idx+1} | 有效均方根能量: {eng:.6f}")
    ...
    def run_step_4(self):
        ...
        self.results["multi_snr"]["recon"] = base.reconstruct_multicomponent_with_snr(
            self.components_cache, self.fs, snr_threshold=3.0
        )

        offset = 100
        for key in ["single_best", "multi_snr"]:
        ...
        ax1.plot(self.results["single_best"]["recon"], color='#d35400')
        ax1.set_title("策略 A: ACMD 单主频最大分量提取波形")

        ax2 = self.figure.add_subplot(212)
        ax2.plot(self.results["multi_snr"]["recon"], color='#16a085')
        ax2.set_title("策略 B: ACMD 多成分 SNR 门限重构波形")
        self.figure.tight_layout()
        self.canvas.draw()
        self.log("重构波形滤波及起始端点区削减完成。")

    def run_step_5(self):
        """步骤 5: FPR 特征峰值比识别与频率计算"""
        if self.results["single_best"]["recon"] is None: return
        self.log("正在注入 FPR 门限识别条件...")

        bpm_s = base.calculate_bpm_fpr(self.results["single_best"]["recon"], self.fs)
        bpm_m = base.calculate_bpm_fpr(self.results["multi_snr"]["recon"], self.fs)

        self.results["single_best"]["bpm"] = bpm_s
        self.results["multi_snr"]["bpm"] = bpm_m

        self.figure.axes[0].set_title(f"策略 A: 单主频极大分量 | 频率: {bpm_s:.2f} BPM")
        self.figure.axes[1].set_title(f"策略 B: 多成分 SNR 重构 | 频率: {bpm_m:.2f} BPM")
        self.canvas.draw()
        self.log(f"最终判定：经典单分量波形法产生呼吸节律：{bpm_s:.2f} BPM; 多成分重构法产生呼吸节律：{bpm_m:.2f} BPM")
