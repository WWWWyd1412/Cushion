# -*- coding: utf-8 -*-
"""
心跳提取对比 v2.0 — 多谐波基线去除 + ACR-BPM + 精细ICA选频
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
v1→v2 改进:
  改进A: 多次谐波去除
         呼吸基频~0.25Hz, 其4次谐波~1.0Hz落入心跳频段
         用 VME 逐一剥离 breath_freq × [1,2,3,4,5]
  改进B: ACR-BPM 心跳版
         自相关搜索0.4-1.5s (40-150BPM)的基础周期
         与FFT-BPM取一致性更高的结果
  改进C: ICA 心跳频段精细选频
         候选分量窗口 [0.9,2.0Hz] 避免呼吸谐波污染边界
  改进D: 去除BPM候选中的呼吸谐波干扰频率
         在FFT峰检测时, 屏蔽 breath_freq×k 附近±0.05Hz的峰
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
K_ROIS   = 4
MIN_DIST = 5
WIN_SEC  = 25.0   # 稍短窗口，30s包含~38次心跳，25s够稳定且更局部
STEP_SEC = 5.0
DEADZONE = 30
CLIP_MAX = 2000
HB_LOW, HB_HIGH = 0.9, 2.0   # 改进C: 收紧边界避开呼吸谐波
...
OUT_DIR   = os.path.join(ROOT, 'Contrast', '心跳', '刘若红_0702_160410_心跳对比_v2')
...
def bpm_acr_hb(sig, fs, min_bpm=BPM_MIN, max_bpm=BPM_MAX):
    """自相关BPM — 搜索0.4–1.5s (40–150BPM)的基础周期"""
    n = len(sig)
    if n < 30: return 0.0
    s   = sig - sig.mean()
...
# ════════════════════════════════════════════════════════════════
# 改进D: FFT-BPM 屏蔽呼吸谐波干扰
# ════════════════════════════════════════════════════════════════
def bpm_fft_clean(sig, fs, breath_freq=0.238,
                  min_bpm=BPM_MIN, max_bpm=BPM_MAX):
    """
    FFT主频法，但屏蔽呼吸基频及其谐波附近±0.06Hz的频率点，
    避免把呼吸4次谐波(~0.95Hz≈57BPM)误判为心跳。
    breath_freq: 呼吸基频(Hz)，默认14.29BPM/60≈0.238Hz
    """
    n     = len(sig)
    ...
    # 屏蔽呼吸谐波
    HARM_BW = 0.06   # 每个谐波屏蔽带宽(Hz)
    for k in range(1, 7):
        harm = breath_freq * k
        harm_mask = np.abs(freqs - harm) < HARM_BW
        mask &= ~harm_mask
...
def best_bpm_v2(sig, fs, breath_freq=0.238):
    """三方投票: FFT清洁版 / ACR / FPR → 取中位数"""
    f  = bpm_fft_clean(sig, fs, breath_freq)
    a  = bpm_acr_hb(sig, fs)
    # FPR
...
def load_ref_ch2(fp):
    ...
    for ln in lines[di:]:
        cols = ln.strip().split('\t')
        try:
            ch1_raw.append(float(cols[0]))   # CH1=RSP
            ch2_raw.append(float(cols[1]))   # CH2=PPG
    ...
    freqs_r = np.fft.rfftfreq(len(ch1_bp), 1.0/fs_rsp)
    psd_r   = np.abs(np.fft.rfft(ch1_bp))**2
    rm = (freqs_r >= 0.1) & (freqs_r <= 0.5)
    breath_freq = float(freqs_r[rm][np.argmax(psd_r[rm])]) if rm.any() else 0.238
    ...
    vote, f, a_, p = best_bpm_v2(ch2_bp, fs_ppg, breath_freq)
    print(f"       参考心率: FFT={f:.1f} ACR={a_:.1f} FPR={p:.1f} 投票={vote:.1f} BPM")
    print(f"       CH2(PPG): {len(ch2)} 点 @ {fs_r:.0f}Hz")
...
# 改进A: 多次谐波VME去除 + ICA融合
# ════════════════════════════════════════════════════════════════
def _vme_multi_harmonic(sig, fs, breath_freq, n_harmonics=5):
    """
    连续剥离呼吸基频及其前n次谐波:
      breath_freq × [1, 2, 3, 4, 5]
    每次用VME_Core提取该频率成分后从信号中减去。
    """
    from algorithms.base import VME_Core
    result = sig.copy()
    for k in range(1, n_harmonics + 1):
        f_harm = breath_freq * k
        if f_harm >= fs / 2 - 0.1:   # 超过 Nyquist 停止
            break
        try:
            u = VME_Core(result - result.mean(), fs=fs,
                         f_init=f_harm, alpha=800)
            result = result - u
        except Exception:
            pass
    return result
...
def _ica_fuse_cardiac(roi_mat, fs):
    """改进C: ICA分量选频窗口收紧到[HB_LOW, HB_HIGH]"""
...
def build_fused_signal(frames, breath_freq=0.238):
    ...
    for roi in rois:
        ...
        ts = ts - np.polyval(np.polyfit(t2, ts, 3), t2)  # poly_detrend
    ...
    fused   = _ica_fuse_cardiac(roi_mat, FS)

    # 改进A: 多谐波去除
    fused = _vme_multi_harmonic(fused, FS, breath_freq, n_harmonics=5)
    # 带通
    fused = butter_bandpass_filter(fused, HB_LOW, HB_HIGH, fs=FS, order=4)
    print(f"       ICA+多谐波VME 融合信号 shape={fused.shape}")
...
def run_all(fused, frames, breath_freq):
    ...
    for name in ALGO_NAMES:
        ...
        try:
            ...
            vote, f, a, p = best_bpm_v2(sig, FS, breath_freq)
        ...
        print(f"  FFT={f:.1f} ACR={a:.1f} FPR={p:.1f} 投票={vote:.1f}  耗时={elapsed:.0f}ms")
    return results
...
# 滑动窗口 BPM
# ════════════════════════════════════════════════════════════════
def sw_bpm(sig, fs, breath_freq):
    win, step = int(WIN_SEC*fs), int(STEP_SEC*fs)
    T, B = [], []
    i = 0
    while i+win <= len(sig):
        v, _, _, _ = best_bpm_v2(sig[i:i+win], fs, breath_freq)
        T.append((i+win/2)/fs);  B.append(v)
        i += step
...
# 单算法独立图（2面板）
# ════════════════════════════════════════════════════════════════
def plot_one(name, res, ref_sig, ref_fs, ref_bpm, breath_freq, out_dir):
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(13, 7),
                                    constrained_layout=True)
    err = abs(res['bpm'] - ref_bpm)
    fig.suptitle(
        f'[{name}]   投票BPM={res["bpm"]:.1f}  |  参考={ref_bpm:.1f}  |  '
        f'误差={err:.1f} BPM  |  耗时={res["time_ms"]:.0f}ms',
        fontsize=11, fontweight='bold'
    )
    rT, rB = sw_bpm(ref_sig, ref_fs, breath_freq)
    aT, aB = sw_bpm(res['sig'], FS, breath_freq)

    ax1.plot(rT, rB, 'r-', lw=2.2, label=f'参考PPG ({ref_bpm:.1f} BPM)')
    ax1.plot(aT, aB, color='#27ae60', lw=1.8, label=f'{name} ({res["bpm"]:.1f} BPM)')
    ax1.axhline(ref_bpm, color='red', lw=0.9, ls='--', alpha=0.45)
    ...
    ax2.plot(t_a, _n(res['sig']), color='#2980b9', lw=1.2, alpha=0.8, label=f'ICA+多谐波VME+{name}')
...
def plot_summary(all_res, ref_bpm, out_dir):
    ...
    ax1.set_title(f'心跳算法对比 v2  (参考={ref_bpm:.1f} BPM, ICA+多谐波VME)')
...
def save_csv(all_res, ref_bpm, out_dir):
    ...
    with open(path, 'w', newline='', encoding='utf-8-sig') as fh:
        w = csv.writer(fh)
        w.writerow(['算法','投票BPM','FFT','ACR','FPR','参考BPM',
                    '绝对误差','相对误差(%)','耗时(ms)'])
...
def main():
    ...
    print('\n'+'='*64)
    print('  心跳提取对比 v2.0  (多谐波VME + ACR + 三方投票)')
    print(f'  输出: {OUT_DIR}')
    print('='*64)
    ...
    print(f"       预处理耗时: {(time.perf_counter()-t0)*1000:.0f}ms")
    ...
    # 控制台汇总
    print('\n'+'='*64)
    print(f'  参考心率(PPG) = {ref_bpm:.1f} BPM  |  呼吸基频 = {breath_freq*60:.1f} BPM')
    print(f'  {"算法":<12}  {"投票BPM":>8}  {"FFT":>7}  {"ACR":>7}  {"误差":>7}  {"耗时ms":>8}')
    print(f'  {"-"*56}')
    for nm in ALGO_NAMES:
        r   = all_res[nm]
        err = abs(r['bpm']-ref_bpm)
        flg = 'OK' if err<=5 else ('~' if err<=10 else 'X')
        print(f'  {nm:<12}  {r["bpm"]:>8.1f}  {r["fft"]:>7.1f}  {r["acr"]:>7.1f}'
