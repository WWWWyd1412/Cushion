#!/usr/bin/env python3
"""
Heartbeat Analyzer — 心跳 (BCG) 信号滑窗分析 GUI
==================================================
基于 cushion 包的新架构入口。

支持算法: EMD / VMD / ACMD / VME
"""
...
# --- cushion 包导入 ---
from cushion.core import load_pressure_txt, get_session_info, clean_dataset
...
# 算法分发映射
ALGO_REGISTRY = {
    "EMD":  lambda frames, fs: extract_emd(frames, fs, freq_band=CFG.FREQ_BAND,
                                            wavelet_alpha=CFG.WAVELET_ALPHA,
                                            use_vme_baseline=True,
                                            use_multicomponent=False),
    "VMD":  lambda frames, fs: extract_vmd(frames, fs, K=CFG.VMD_K, alpha=CFG.VMD_ALPHA,
                                            freq_band=CFG.FREQ_BAND,
                                            wavelet_alpha=CFG.WAVELET_ALPHA,
                                            use_vme_baseline=True,
                                            use_multicomponent=False),
    "ACMD": lambda frames, fs: extract_acmd(frames, fs),
    "VME":  lambda frames, fs: extract_vme(frames, fs),
}


class HeartbeatAnalyzerWindow(QMainWindow):
    """心跳 (BCG) 信号滑窗分析主界面"""

    def __init__(self):
        super().__init__()
        self.setWindowTitle("心跳算法滑窗分析测试工具 (EMD/VMD/ACMD/VME)")
    ...
    def setup_ui(self):
        ...
        self.bpm_selector.addItems(["FPR (特征点法)", "Peak (常规波峰法)"])
        ...
        layout_w.addWidget(QLabel("窗口 (秒):"))
        self.edit_window = QLineEdit("15")
        layout_w.addWidget(self.edit_window)
        layout_w.addWidget(QLabel("步长 (秒):"))
        self.edit_step = QLineEdit("3")
    ...
    def load_and_clean(self):
        ...
        self.log(f"清洗完成: {len(self.clean_frames)} 帧有效")
    ...
    def run_analysis(self):
        ...
        self.sliding_results = []
        self.log(f"算法: {algo_name} | BPM: {'FPR' if use_fpr else 'Peak'}")
        ...
        if use_sliding:
            window_sec = float(self.edit_window.text())
            step_sec = float(self.edit_step.text())
            ...
            self.log(f"滑窗: {len(windows)} 个窗口, {window_sec}s/窗口, {step_sec}s/步长")
            ...
            for idx, (start, end) in enumerate(windows):
                ...
                if use_fpr:
                    ...
                    bpm = calculate_bpm_peak(wave, self.fs,
                                              min_dist_sec=CFG.BPM_MIN_DIST_SEC,
            ...
            self.log(f"HR 统计: 均值={stats['mean']:.1f}, 标准差={stats['std']:.1f}, "
    ...
    def plot_window(self):
        ...
        ax_trend.plot(times_sec, bpm_vals, color='#ef4444', marker='o',
                       markersize=6, linewidth=2, label='HR 趋势')

        sel_time = times_sec[idx]
        sel_bpm = bpm_vals[idx]
        ax_trend.scatter(sel_time, sel_bpm, color='#3b82f6', s=120, zorder=5,
                          label='当前选中窗口')
        ax_trend.axvline(x=sel_time, color='#3b82f6', linestyle='--', alpha=0.7)

        valid = [b for b in bpm_vals if b > 0]
        if valid:
            ax_trend.axhline(y=np.mean(valid), color='#f59e0b',
                              linestyle='--', label=f'均值 HR: {np.mean(valid):.1f}')

        ax_trend.set_title("心率 (BPM) 变化趋势", color='#e4e4e7',
        ...
        ax_wave.plot(time_axis, wave, color='#ef4444', linewidth=2,
                      label='提取的心搏分量')

        # J-peak 标注
        peaks, _ = find_peaks(wave, distance=int(self.fs * 0.4))
        if len(peaks) > 0:
            ax_wave.plot(time_axis[peaks], wave[peaks], 'bo', markersize=5, alpha=0.8,
                          label=f'J 峰 ({len(peaks)} 个)')

        ax_wave.set_title(
            f"分帧明细: {start_time:.1f}s - {end_time:.1f}s | HR: {bpm:.1f} BPM",
        ...
        ax_psd.axvspan(0.8, 2.2, color='red', alpha=0.1, label="心搏带 (0.8-2.2 Hz)")
        ax_psd.set_xlim(0, 3.0)
        ax_psd.set_title(f"当前分帧 PSD | 心率: {bpm:.1f} BPM",
                          color='#e4e4e7', fontsize=11, fontweight='bold')
        ...
        self._mark_psd_peak(ax_psd, f, psd, (0.8, 2.2), label="心率")
...
def main():
    ...
    window = HeartbeatAnalyzerWindow()
    window.show()
    sys.exit(app.exec_())
